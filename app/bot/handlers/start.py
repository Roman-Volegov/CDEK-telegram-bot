...
HELP_TEXT = (
    ...
    "• <b>Мои заказы</b> — статусы СДЭК и повторная выгрузка PDF\n"
    "• <b>Оплатить за СДЭК</b> — сумма неоплаченных заказов и подтверждение оплаты\n"
    "• Админ может выбрать пользователя в «Мои заказы» и «Оплатить за СДЭК»\n\n"
    "PDF: отправьте номер <code>2026-000001</code> или <code>/pdf 2026-000001</code>.\n"
...
async def cmd_start(
    ...
    if not decision.allowed:
        ...
        if decision.status == STATUS_REJECTED and not decision.can_request:
            ...
            return
        await message.answer(
            "Привет! Это бот для работы с доставкой <b>СДЭК</b>.\n\n"
            "Он умеет:\n"
            "• распознать адрес в любом формате через сервис DaData "
            "и посчитать стоимость доставки\n"
            "• создать заказ в СДЭК и прислать PDF накладной и штрихкодов\n"
            "• показать ваши заказы со статусом и ссылкой на сайт СДЭК\n"
            "• посчитать сумму к оплате за доставку СДЭК\n\n"
